...
def elements_atk_catt(l_tag: Union[Tag | list[Tag | NavigableString]], control:int=None):
	"""
	...
	if isinstance(l_tag, Tag):
		for n,i in enumerate(text):
			type_text = l_tag['src']
			if i in type_text:
				return str(text[n].capitalize())

	if isinstance(l_tag, list):
		for n, i in enumerate(l_tag):
			try:
				catt_text = i['src']
			...
			for description in category:
				if description in catt_text:
					l_tag[n] = description.lower()
					break
		return (l_tag)
...
def list_of_elements(location:ResultSet[Tag], gen:int):
	...
	for tag in location:
		...
		try:
			...
			type_text = elements_atk_catt(a_tag)
			types.append(type_text)
